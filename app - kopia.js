...
//Hard coded string for table booking for debug purposes. To be set dynamically after film is decided and availible restaurant table presentation
var dayTimeStringTableBooking = "lor1820";

//Books a table and receives a response html page from the server
var bookingATableNow = Promise.all([restaurantUrl,redirectedBookingSubUrlAndCookieEx,availibleTablesList,dayTimeStringTableBooking])
                              .then(function(results){

                                var fullBookingUrlString = results[0]+"/"+results[1][0];
                                fullBookingUrlString = fullBookingUrlString.trim().replace(/['"]+/g, '');
                                var CookieString = results[1][1];
                                CookieString = CookieString.replace(/['"]+/g, '');
                            //    console.log("fullBookingUrlString", fullBookingUrlString);
                            //    console.log("CookieString", CookieString);
                                var avalibeTables = results[2];
                                var lengthAvailibleTables =  avalibeTables.length;
                              //  console.log("lengthAvailibleTables ", lengthAvailibleTables);
                                var csrftoken = avalibeTables[lengthAvailibleTables -2];
                              //  console.log("avalibeTables", avalibeTables);
                              //  console.log("csrftoken", csrftoken);
                                var dayTimeStr = results[3];
                                //console.log("dayTimeStr",dayTimeStr);
                                return bookATable(fullBookingUrlString,CookieString, "group1",dayTimeStr,csrftoken);
                              }).then(function(html){
...
// Extracts "Din bokning är gjord" from the previous received response, that is from the html page which the server responded to the booking POST.
var bookingResponse = bookingATableNow.then(function(html){
